...
The Variable for S04 are variable 01 and variable 02. variable one has two main phases with one early plateau and a hump of rapid growth towards the end. Variable two looks mostly like white noise with a hump towards the beginning with what may be a steep hump towards the end and a more middle hump towards the middle. However these humps are difficult to pick out because of the noise.
...
comparing the MAPE output we found that the naive model was best at predicting both variables one and two and will be using these for forecasting..
...
Data set S06 contains two strikingly similar data sets var06 and var07. Each shows a mostly linear patterns with a few humps with larger increases and dramatic dips with one point that is drastically larger then any other. the autocorrelation showed that between time points there is a very high level of correlation hovering around one for both of these time points as seen in the ACF plot. to help make clean prediction for these variables the single high value was omitted from the data set and all five empty values were imputed using linear imputation. While the graphs look extraordinarily similar these plots data sets ar not identicle
...
It seems after testing all of these models most of them perform relatively similar with ARIMA being the worst model in both variable cases and SES being the best for var05 and ets being the best for var07. However these are such small differences that these may be performing the same and just one is effected more because of the randomness in data partitions. altogether we chose to move forward to forcast with the SES for variable 05 and ETS for variable 07
